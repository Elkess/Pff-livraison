<x-layout>
    <div class=" ">
        <a href="{{ route('admin.users.create') }}" class="ml-4 new-user-btn bg-blue-600 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded">New user</a>
        <table class=" m-4 border-collapse border border-black ">
            <thead>
                <tr>
                    <th class="border border-black px-4 py-2 bg-black text-white">Name</th>
                ...
                @foreach ($users as $user)
                    <tr class="bg-white hover:bg-blue-50">
                        ...
                        border border-black px-4 py-2">
                            <a href="{{ route('admin.users.show', $user) }}" class="user-view-button hover:bg-blue-200  text-blue-600 font-bold py-1 px-2 rounded">View</a>
                            <a href="{{ route('admin.users.edit', $user) }}" class="user-edit-button hover:bg-green-200 text-green-600 font-bold py-1 px-2 rounded">Edit</a>
                            <form action="{{ route('admin.users.destroy', $user) }}" method="post" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="user-delete-button hover:bg-red-200  text-red-600 font-bold py-1 px-2 rounded">Delete</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
